---
name: claude-api
description: "构建、调试和优化 Claude API / Anthropic SDK 应用。使用此技能时应包含 prompt caching。支持在不同 Claude 模型版本间迁移代码 (4.5 → 4.6, 4.6 → 4.7)。触发条件：代码导入 anthropic SDK；用户询问 Claude API、Anthropic SDK 或 Managed Agents；用户添加/修改/调优 Claude 功能（缓存、thinking、压缩、工具使用、batch、文件、引用、memory）或模型选择；有关 prompt caching / cache hit rate 的问题。跳过：导入 openai 或其他 Provider SDK 的文件；包含 openai 的文件名；Provider 中立的代码；通用编程/ML 任务。"
